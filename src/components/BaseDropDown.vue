<template>
  ...
  >
    <div
      ...
      @click="open = !open"
    >
      <span v-for="each in multiSelected" :key="each"
        >{{ each.name }}
        <span
          @click.stop="removeItem(each)"
          id="x-circle"
          class="rounded-circle"
          >&times;</span
        >
        {{ multiSelected[multiSelected.length - 1] === each ? " " : ", " }}
      </span>
    </div>
    <div
      v-else
    ...
    <div class="items" :class="{ selectHide: !open }">
      <div v-for="(option, i) of options" :key="i" @click="sendData(option)">
        {{ option.name }}
      </div>
    </div>
  </div>
